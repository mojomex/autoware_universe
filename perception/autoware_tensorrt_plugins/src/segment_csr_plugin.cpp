...
{
  const auto num_rows = static_cast<std::int32_t>(input_desc[0].dims.d[0]);
  const auto num_cols = static_cast<std::int32_t>(input_desc[0].dims.d[1]);
  const auto indptr_size = static_cast<std::int32_t>(input_desc[1].dims.d[0]);
